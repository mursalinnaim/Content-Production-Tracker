...
describe('content plan review', () => {
    ...
    it('saves an edited draft without starting another generation', async () => {
        ...
        await flushPromises();
        await wrapper.get('button', { text: /Content Generation/ }).trigger('click');
        await wrapper.get('button', { text: 'Edit' }).trigger('click');
        await flushPromises();
        if (
            wrapper
                .get('button', { text: /Content Generation/ })
        ...
        await wrapper.get('button', { text: 'Save draft' }).trigger('click');
    ...
    it('accepts the current completed version', async () => {
        ...
        await wrapper
            ...
            .trigger('click');
        await wrapper
            .get('button', { text: 'Accept this plan' })
            .trigger('click');
        await flushPromises();
    ...
    it('regenerates with explicit instructions and starts polling the new generation', async () => {
        ...
        await wrapper.get('button', { text: 'Regenerate' }).trigger('click');
        await flushPromises();
    ...
    it('warns before discarding unsaved edits when switching generations', async () => {
        ...
        await wrapper.get('button', { text: /Generation #1/ }).trigger('click');

        expect(confirm).toHaveBeenCalled();
        expect(wrapper.text()).toContain('Unsaved title');
    });
});
